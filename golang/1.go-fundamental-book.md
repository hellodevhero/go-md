...
Before we start coding, know these commands — you will use them daily:
...
`go mod init` creates a `go.mod` file, which defines your **module** — a collection of related Go packages with a name and dependency tracking. Think of it like `package.json` in Node.js or `requirements.txt` in Python, but built into the language. Every Go project needs a module.
...
- `go build -o myapp` compiles your code into a standalone binary — no runtime or VM needed. You can copy that single file to any machine with the same OS/architecture and it runs
...
**`byte` vs `rune` — when to use which:**
- `byte` is an alias for `uint8` (0–255). Use it for ASCII characters and raw binary data.
- `rune` is an alias for `int32`. It represents a single Unicode code point — any character in any language.
- Go strings are sequences of bytes internally, but when you iterate with `range`, Go decodes them into runes automatically:
...
**Rule of thumb:** Use `byte` for ASCII/binary, `rune` for text processing with international characters.
...
**Gotcha #2 — Avoid defer inside loops:**
...
**Gotcha — Slices share the same backing array:**
...
When you slice a slice, both point to the same underlying array. To get an independent copy, use `copy` or the full slice expression `original[1:3:3]` (limits capacity).
...
Use `var s []int` (nil) as the default. Both nil and empty slices work with `len`, `append`, and `range`.

> **Since Go 1.21:** The `slices` package in the standard library provides useful helpers like `slices.Contains`, `slices.Sort`, `slices.Index`, and more. Import it with `import "slices"`.
...
**Gotcha #1 — Writing to a nil map panics:**
...
Always initialize maps with `make(map[string]int)` or a literal before writing.

**Gotcha #2 — Maps are NOT safe for concurrent access.** If multiple goroutines read and write to the same map simultaneously, your program will crash. You need `sync.Mutex` or `sync.Map` (covered in Week 5).

> **Since Go 1.21:** The `maps` package provides helpers like `maps.Keys`, `maps.Values`, `maps.Clone`, and `maps.Equal`. Import it with `import "maps"`.
...
Struct tags are metadata strings attached to struct fields. They are used by packages like `encoding/json` to control how fields are serialized.
...
Tags are also used by database libraries (`db:"column_name"`), validation libraries (`validate:"required"`), and more. We will use them extensively in later chapters.

### Constructor Convention — `NewXxx`

Go has no constructors. The convention is to create a function named `NewXxx` that returns an initialized struct:
...
This is especially important when the struct has fields that need validation or default values.
...
**Gotcha — nil interface vs interface holding nil value:**

This is one of Go's most notorious traps. An interface holding a nil concrete value is **NOT** equal to nil:
...
**Why?** An interface value is nil only when both its type and value are nil. Here the type is `*MyError` (not nil), even though the value is nil.

**Fix:** Return `nil` directly, not a nil pointer through an interface:
...
Every package can have one or more `init()` functions. They run automatically before `main()`, in the order packages are imported.
...
**Why blank imports exist:** Some packages only need their `init()` to run. For example, database drivers register themselves in `init()`:
...
Without the blank import, Go would not compile (unused imports are errors), and the driver would never register itself.

**Rule:** Don't overuse `init()`. It runs implicitly, making code harder to follow. Prefer explicit initialization in `main()` when possible.
...
**When to use generics vs when to use interfaces:**
- **Use generics** for data structures (Stack, Set, Map), utility functions (Min, Max, Filter, Map), and when you need type-safe collections.
- **Use interfaces** for behavior abstraction (Reader, Writer, Handler), dependency injection, and when different types need different implementations.
- **Don't use generics** just because you can. If an interface works, use an interface. Generics add complexity — earn it.
...
Define package-level errors that callers can check with `errors.Is`:
...
This principle makes your code flexible and testable:
...
- **Accept interfaces:** Callers can pass any implementation (real DB, fake for testing).
- **Return structs:** Callers know exactly what they get — no need to guess what is behind the interface.
